...
// @updateURL   https://static.yaoonion.fun/better-hetao/pub/script.user.js
// @downloadURL https://static.yaoonion.fun/better-hetao/pub/script.user.js
// ==/UserScript==
"use strict";
(() => {
  // dist/config.js
  ...
  function getAuthorization() {
    const manual = GM_getValue("authorization", "");
    if (manual)
      return manual;
    const ls = localStorage.getItem("authorization");
    if (ls)
      return ls;
    const ss = sessionStorage.getItem("authorization");
    if (ss)
      return ss;
...
${description}`,
  ...
  async function requestProblemDetail(url, headers) {
    try {
...
${text}`);
      ...
      log("Fetch failed, falling back to GM_xmlhttpRequest:", fetchErr);
    }
...
${response.responseText}`);
  ...
  async function copyMarkdown() {
    ...
    try {
      const data = await requestProblemDetail(url, headers);
...
\u9519\u8BEF\u8BE6\u60C5: ${message}`, 5e3);
  ...
  // dist/authInterceptor.js
  var AUTH_EVENT_TYPE = "better-hetao:auth";
  function initAuthInterceptor() {
    const script = document.createElement("script");
    script.textContent = `
    (function () {
      const AUTH_EVENT_TYPE = '${AUTH_EVENT_TYPE}';

      function reportAuth(token) {
        if (!token) return;
        document.dispatchEvent(new CustomEvent(AUTH_EVENT_TYPE, { detail: token }));
      }
      ...
      function extractAuthFromHeaders(headers) {
        ...
        if (Array.isArray(headers)) {
          const item = headers.find(h => Array.isArray(h) && h[0].toLowerCase() === 'authorization');
      ...
      window.fetch = async function (...args) {
        try {
          ...
          if (typeof url === 'string' && url.includes('api.htoj.com.cn')) {
            reportAuth(extractAuthFromHeaders(init && init.headers));
          }
        } catch (e) {
          // ignore
        }
      ...
      XMLHttpRequest.prototype.setRequestHeader = function (header, value) {
        try {
          if (
            ...
            typeof this._bhUrl === 'string' &&
            this._bhUrl.includes('api.htoj.com.cn')
          ) {
            reportAuth(value);
          }
        } catch (e) {
  ...
  `;
    ...
    document.addEventListener(AUTH_EVENT_TYPE, (e) => {
      const token = e.detail;
      if (token) {
        GM_setValue("authorization", token);
        log("Authorization token captured from page request.");
      }
    });
  }
